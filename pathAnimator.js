...
function interpretCarLine(time, line) {
    ...
    let idType = line[0] + _idSep + interpretLocoType(line[1]);
    let result = [
        parseFloat(line[2]),
        parseFloat(line[3]),
        parseFloat(line[4]),
...
/**
 * interprets custom loco type based on their ID (assuming that id is fixed). Also allows the processing of vanilla locos, but they will be returned as is.
 * @param {any} name
 * @returns {String} name for loco
 */
function interpretLocoType(name) {
    if ( name.includes("DE2") | name.includes("DE6") | name.includes("SH282")) {
        return name;
    }
    else if (_modLocos.hasOwnProperty(name)) {
        return _modLocos[name];
    }
    return "ID not found";
}
